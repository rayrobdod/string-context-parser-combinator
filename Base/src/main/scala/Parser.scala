...
trait Parser[Expr, +A] {
	...
	def andThen[B, Z](rhs:Parser[Expr, B])(implicit ev:typelevel.Sequenced[A,B,Z]):Parser[Expr, Z] = parsers.AndThen(this, rhs, ev)
	...
	def optionally[Z](implicit ev:typelevel.Optionally[A, Z]):Parser[Expr, Z] = parsers.Optionally(this, ev)
}
