...
object AssembleUnapply {
	def zero[UnexprA](c:Context)(
		...
		val extractorName = c.freshName(c.universe.TypeName("extractor"))
		...
		c.Expr[Boolean](
			q"""
				{
					final class $extractorName extends com.rayrobdod.stringContextParserCombinator.Unapply.Zero[$valueType] {
						def unapply($valueName: $valueType):Boolean = $condition
					}
					new $extractorName
				}.unapply($value)
			"""
	...
	def one[UnexprA](c:Context)(
		...
		c.Expr(
			q"""
				{
					final class $extractorName extends com.rayrobdod.stringContextParserCombinator.Unapply.Fixed[$valueType, $extractedType] {
						def unapply($valueName: $valueType):Option[$extractedType] = {
							if ($condition) {
	...
	def many[UnexprA](c:Context)(
		...
		c.Expr[Any](
			q"""
				{
					final class $extractorName extends com.rayrobdod.stringContextParserCombinator.Unapply.Fixed[$valueType, $extractedType] {
						def unapply($valueName: $valueType) = {
							if ($condition) {
