...
trait Parsers {
	...
	def OfType[A](using tpe:Type[A])(using Quotes):Parser[Expr[A]] = parsers.OfType(tpe)
	 ...
	 */
	def Lifted[Lifter[A], Z](lifterType:TypeFunction[Lifter], lift:LiftFunction[Lifter, Z], description:Expecting)(using Quotes):Parser[Expr[Z]] = parsers.Lifted(lifterType, lift, description)
