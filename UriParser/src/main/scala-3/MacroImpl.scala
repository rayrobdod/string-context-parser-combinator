...
object MacroImpl {
	...
	def stringContext_uri(sc:Expr[scala.StringContext], args:Expr[Seq[Any]])(using Quotes):Expr[URI] = {
		object ParserPieces {
			...
			val nullExpr:Expr[Null] = '{ null }
			implicit val AndThenCodepointString:typelevel.Sequenced[CodePoint, String, String] = {
				(a:CodePoint, b:String) => s"${a}${b}"
			}
			implicit val AndThenStringCodepoint:typelevel.Sequenced[String, CodePoint, String] = {
				(a:String, b:CodePoint) => s"${a}${b}"
			}
			implicit val AndThenStringString:typelevel.Sequenced[String, String, String] = {
			...
			val HostPortP:Parser[(Expr[String], Expr[Int])] = {
			...
			val ServerP:Parser[(Expr[String], (Expr[String], Expr[Int]))] =
				(UserInfoP andThen IsString("@")).optionally.map(_.getOrElse(nullExpr)) andThen HostPortP

			val OpaquePartP:Parser[Expr[String]] = {
			...
			val AbsPathExprP:Parser[Expr[String]] = AbsPathP.map(Expr.apply _)


			val FragmentOrQueryString:Parser[Expr[String]] = {
			...
			val QueryP:Parser[Expr[String|Null]] = (IsString("?") andThen FragmentOrQueryString).optionally.map(_.getOrElse(nullExpr))
			val FragmentP:Parser[Expr[String|Null]] = (IsString("#") andThen FragmentOrQueryString).optionally.map(_.getOrElse(nullExpr))


			val RelPathP:Parser[String] =
				(EscapedChar orElse UnreservedChar orElse CodePointIn(";@&=+$,")).repeat(1) andThen AbsPathP.optionally
			val NetPathP:Parser[((Expr[String], (Expr[String], Expr[Int])), Expr[String])] = IsString("//") andThen ServerP andThen AbsPathExprP
			val noServer:(Expr[String], (Expr[String], Expr[Int])) = (nullExpr, (nullExpr, Expr.apply(-1)))

			val HierarchialPart:Parser[(((Expr[String], (Expr[String], Expr[Int])), Expr[String]), Expr[String])] = {
				(NetPathP orElse AbsPathExprP.map(x => (noServer, x))) andThen QueryP
			}

			val AbsoluteUriP:Parser[Expr[URI]] = {
				SchemeP andThen
				IsString(":") flatMap
				({(scheme:Expr[String]) =>
					(IsString("//") andThen
			...
			val RelativeUriP:Parser[Expr[URI]] = {
				((NetPathP
			...
			val ResolvedUriP:Parser[Expr[URI]] = {
			...
			val Aggregate:Parser[Expr[URI]] = (ResolvedUriP orElse AbsoluteUriP orElse RelativeUriP) andThen End()
		}

		macroimpl[URI](ParserPieces.Aggregate)(sc, args)
	}
}
